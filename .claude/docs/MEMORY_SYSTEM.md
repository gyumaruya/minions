...
### Phase 1: シンプルな JSONL ベース（✅ 実装済み）

**現在の実装:**
- Memory Broker は未実装
- シンプルな JSONL append-only ログ
- mem0 ベクトル索引は未使用

**記憶パス:**
```
~/.config/ai/memory/events.jsonl  # グローバル記憶（全プロジェクト共通）
```

**実装済みフック:**
- `load-memories` - セッション開始時に記憶読み込み
- `auto-learn` - ユーザー修正の自動学習
- `pre-tool-recall` - ツール実行前の記憶参照
- `post-tool-record` - ツール実行結果の記録

**スキーマ:**
- `hook-memory` クレート（Rust）で定義
- `MemoryEvent`, `MemoryType`, `MemoryScope`, `AgentType`

### 今後の拡張

1. **Phase 2: ローカル記憶の追加** - プロジェクト固有の記憶分離
2. **Memory Broker**: このドキュメントで設計された統一API
3. **mem0 統合**: ベクトル検索による意味的検索
4. **自動要約**: 長い記憶を自動で要約
5. **忘却ポリシー**: TTL + 利用頻度で自動整理
6. **ゴールデンメモリ**: 重要な記憶を固定
7. **クロスプロジェクト**: 複数プロジェクト間の記憶共有（Phase 1 で達成）
